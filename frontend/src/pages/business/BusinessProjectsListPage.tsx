...
export function BusinessProjectsListPage() {
	...
	if (projects.length === 0) {
		return (
			<div className="mx-auto max-w-5xl p-4 sm:p-6">
				<Card className="space-y-3 border border-slate-800 bg-slate-900/60 p-6 shadow-xl text-center">
					<h3 className="text-lg font-semibold text-slate-50">No projects yet</h3>
					<p className="text-sm text-slate-300">You haven't created any projects yet. Create a project to get applications from students.</p>
					<div className="mt-4 flex justify-center">
						<Button onClick={() => navigate("/business/projects/new")} disabled={!canCreate}>Create project</Button>
					</div>
					{!canCreate && <p className="text-xs text-slate-400 mt-2">Project creation is disabled for your account. Contact support or verify your business profile.</p>}
				</Card>
			</div>
		);
	}
		...
		<div className="space-y-6 animate-page-enter">
			<header className="flex items-center justify-between">
				<div className="space-y-1">
					...
					<p className="text-sm text-slate-300">Create and manage projects to assign to students.</p>
				</div>
				<Button asChild>
			...
			<div className="space-y-3">
				{projects.map((project) => (
					<Card key={project.id} className="border border-slate-800 bg-slate-900/80 p-4 shadow-xl">
